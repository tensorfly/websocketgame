...
/*****************************************************/
		game.on('forward', function(res){
			console.log('forward command received!');
			self.key.forward = res.forward;
		});
		game.on('turn', function(res){
			//console.log('al: %s, beta: %s, gm: %s', res.al, res.beta, res.gm);
			if (res.beta>5) {
				self.key.left = true;
